...
const showProducts = (products) => {
  ...
  for (const product of allProducts) {
    ...
    <img class="product-image" src=${product.image}></img>
      ...
      <p class="text-center">product rating:(${product.rating.rate}) <br> Total ratings:(${product.rating.count})</p>
...
const updatePrice = (id, value) => {
  ...
  document.getElementById(id).innerText = Math.max(total).toFixed(2);
};

// set innerText function
const setInnerText = (id, value) => {
  document.getElementById(id).innerText = Math.max(value).toFixed(2);
};
